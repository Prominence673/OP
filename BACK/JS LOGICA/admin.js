document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderTable(tipo, data) {
    ...
    data.forEach(row => {
      ...
      html += `<td>
        <button type="button" class="btn-accion btn-eliminar" title="Eliminar" data-id="${row.id_autos || row.id_paquetes || row.id_estadias || row.id_pasajes}" data-tipo="${tipo}" onclick="abrirEliminarProducto('${tipo}', '${row.id_autos || row.id_paquetes || row.id_estadias || row.id_pasajes}')">🗑️</button>
        <button type="button" class="btn-accion btn-modificar" title="Modificar" data-id="${row.id_autos || row.id_paquetes || row.id_estadias || row.id_pasajes}" data-tipo="${tipo}" onclick="abrirModificarProducto('${tipo}', '${row.id_autos || row.id_paquetes || row.id_estadias || row.id_pasajes}')">✏️</button>
        <button type="button" class="btn-accion btn-agregar" title="Añadir" data-tipo="${tipo}" onclick="abrirAgregarProducto('${tipo}')">＋</button>
      </td>`;
      html += "</tr>";
    });
    html += "</tbody></table>";
...
function abrirEliminarProducto(tabla, id) {
  document.querySelectorAll('#productos .contenido-subseccion').forEach(sec => sec.hidden = true);
